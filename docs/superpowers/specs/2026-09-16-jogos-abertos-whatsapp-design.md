...
- Formulário rápido de publicação: escolher a data, definir uma vez o preço por jogador do lote (opcional), depois adicionar N linhas "hora início – hora fim". Um botão "Publicar no WhatsApp" insere as N `games` rows de uma vez, todas com o mesmo `open_batch_id`.
- **Correção (2026-09-16, ao rever a implementação):** a versão anterior desta linha previa também formato, restrição de género e um toggle ranked/amigável como defaults do lote — descontinuado, não construído. Motivos: (1) `format` é inerte para um jogo em aberto (`num_courts=1`/`max_players=4`, portanto um único jogo — `sobe_desce` vs `todos_contra_todos` não têm diferença observável com só 4 jogadores); (2) uma restrição de género por omissão vai contra o espírito de "qualquer pessoa pode entrar e fechar campos" desta funcionalidade; (3) `ranked`/amigável não tem coluna própria em `games` (só existe em `group_matches`) — implementá-lo exigiria trabalho de schema fora do âmbito deste plano. Se um clube pedir estas opções no futuro, tratar como pedido novo, não como bug desta versão.
