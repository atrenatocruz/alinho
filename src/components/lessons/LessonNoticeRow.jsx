...
export default function LessonNoticeRow({ notice, onOpen }) {
  ...
  const vars = {
    ...
    when: d.lesson_date ? formatDate(d.lesson_date, i18n.language, { weekday: 'long', day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' }) : '',
    reason: d.reason ? t(`lessons.reason_${d.reason}`).toLowerCase() : '',
  }
  const bad = notice.kind === 'lesson_cancelled' || notice.kind === 'lesson_enrolment_rejected'
  return (
