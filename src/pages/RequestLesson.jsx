          ...
          <div className="flex flex-wrap gap-2">
            {blocks.map((b) => (
              <button key={b.key} type="button" aria-pressed={b.key === blockKey} onClick={() => chooseBlock(b.key)}
                className={`text-left rounded-2xl border px-3 py-1.5 transition-colors duration-fast ${b.key === blockKey ? 'border-ink-900 bg-ink-900 text-white' : 'border-line bg-canvas text-ink-900'}`}>
